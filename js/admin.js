...
async function saveEmployee(){
    ...
    if(!name || !username || !password){
        ...
        showWarning("يرجى إدخال جميع البيانات.");
...
async function deleteEmployee(id){
    ...
    showDeleteConfirm("هل تريد حذف هذا الموظف؟ لا يمكن التراجع عن هذه العملية.", async () => { const result=await api("deleteEmployee",{id}); if(result.success){await employeesPage();showSuccess("تم حذف الموظف بنجاح.");}else{showError(result.message||"تعذر حذف الموظف.");} });
...
async function deleteReport(id){
    ...
    showDeleteConfirm("هل تريد حذف هذا التقرير؟ لا يمكن التراجع عن هذه العملية.", async () => { const result=await api("deleteReport",{id}); if(result.success){showSuccess("تم حذف التقرير بنجاح.");reportsPage();}else{showError(result.message||"تعذر حذف التقرير.");} });
...
document.addEventListener("click",function(e){const link=e.target.closest("a.view-btn");if(link&&typeof openPdfViewer==="function"){e.preventDefault();openPdfViewer(link.getAttribute("href"),link.textContent.trim()||"التقرير الأسبوعي");}});
